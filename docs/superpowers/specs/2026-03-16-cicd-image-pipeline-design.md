...
단계적으로 인터페이스를 확장한다. Core를 먼저 구현하고, 이후 CLI → API → MCP 순으로 확장한다.

| 단계 | 인터페이스 | 설명 |
|------|-----------|------|
| 1 | **Core** | 핵심 에이전트 로직 (Intent Parser, Plan Generator, Execution Engine, Tools) |
...
- 인증 정보는 `registry_type`에 따라 환경변수에서 자동으로 로드한다 (파라미터로 전달하지 않음)
...
4. step당 최대 3회 시도 (초기 1회 + 재시도 2회). `attempt_number > max_attempts`이면 중단
